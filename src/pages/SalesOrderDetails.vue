      ...
      <div v-for="(items, PoNr) in dateGroups" :key="PoNr">
        <q-card flat bordered class="q-mb-md">
          <q-card-section class="bg-primary text-white q-py-xs">
            <div class="text-h7">Ordrenr: {{ poNr }}</div>
            <div class="text-h7">Kunde: {{ items[0].navn }}</div>
            <div class="text-h7">Selger: {{ items[0].selgernavn }}</div>
            <div class="text-subtitle2">Skipningsdato: {{ date }}</div>
          </q-card-section>
          ...
          <q-card-section class="q-py-xs"> <!-- Redusert padding her -->
            <div class="row items-center">
              <div class="col-6 text-caption">Varenr</div>
...
export default {
  ...
  methods: {
    ...
    groupItems (items) {
      const grouped = {}
      items.forEach(item => {
        const date = this.formatDate(item.leveringsdato)
