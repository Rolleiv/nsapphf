     ...
     <template v-slot:body-cell-action="props">
        <q-td :props="props">
          <q-btn
            flat
            icon="edit"
            @click.stop="openOrderDetails(props.row.ordrenr)">
          </q-btn>
        </q-td>
      </template>
    </q-table>
...
<script>
const FETCH_ORDRER_URL = `${process.env.API}/Salesorderlist/Retrieve`
export default {
  name: 'Temperaturer',
      ...
      columns: [
        {
          ...
          sortable: false,
          align: 'center',
          format: (val) => val, // Du trenger ikke å formatere denne verdien, men Vue krever at format funksjonen eksisterer
          style: 'width: 100px'
        },
        {
          name: 'Dato',
        ...
        },
        {
          name: 'Ordrenr',
          label: 'Ordrenr',
          field: 'ordrenr', // Endret fra 'varelinjebelopnto' til 'dato'
          // align: 'left',
          sortable: true
          // format: (val) => this.formatDate(val) // legg til hvis du vil formatere datoen
        },
  ...
  methods: {
    ...
    openOrderDetails (ordrenr) {
      this.$router.push({ name: 'order-details', params: { ordrenr: ordrenr } })
      // this.$router.push({ name: '/salesorderdetails', params: { ordrenr } })
    },
    groupItemsByOrderNr (items) {
      const grouped = items.reduce((acc, item) => {
      // Anta at `ordrenr` er identifikatoren for hver ordre
        if (!acc[item.ordrenr]) {
          acc[item.ordrenr] = { ...item, count: 1 } // Initialiser med første forekomst av item
        } else {
        // Oppdater eksisterende gruppe med informasjon, om nødvendig
          acc[item.ordrenr].count += 1 // Eksempel på hvordan man kan telle antall linjer per ordre
        // Her kan du legge til logikk for å summere beløp eller annen aggregering
        }
        return acc
      }, {})

      // Returner et array av de grupperte objektene
  ...
  async mounted () {
    try {
      ...
      if (response.ok) {
        ...
        // Grupperer items etter ordrenr
        this.originalItems = this.groupItemsByOrderNr(data)
        this.filteredItems = this.originalItems // Initialiser med de grupperte dataene
      } else {
