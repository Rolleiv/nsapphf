...
<q-table :rows="filteredRows" :columns="columns" v-model:pagination="pagination" dense row-key="id" :loading="loading" binary-state-sort>  <!-- Tilpasset slot for tabellinnhold -->
...
<script>
// const FETCH_ORDRER_URL = 'http://localhost:8080/api/Tidtilstede/Retrieve'
// const FETCH_ORDRER_URL = 'https://nss.norsksjomat.no/nsapi/api/Tidtilstede/Retrieve'
const FETCH_ORDRER_URL = `${process.env.API}/Tidtilstede/Retrieve`
export default {
  data () {
    return {
      columns: [
        { name: 'ansattnr', align: 'left', label: 'Ansattnr', field: 'c_T701Timetransaksjoner_Ansattnr' },
        { name: 'navn', align: 'left', label: 'Navn', field: 'c_Sr001Navn_Navn' },
        { name: 'orginaltidinn', align: 'left', label: 'Tid Inn', field: 'c_T701Timetransaksjoner_Orginaltidinn' },
        { name: 'orginaltidut', align: 'left', label: 'Tid Ut', field: 'c_T701Timetransaksjoner_Orginaltidut', format: val => val || 'N/A' },
